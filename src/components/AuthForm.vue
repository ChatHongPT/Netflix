...
<template>
  <form @submit.prevent="onSubmit" class="space-y-6 transition-all duration-300">
    <div class="space-y-4">
      ...
      <div class="transform transition-all duration-300">
        <label class="block text-sm font-medium text-gray-300">비밀번호 {{ props.mode === 'signup' ? '(TMDB API 키)' : '' }}</label>
        <input
          type="password"
          v-model="password"
          class="mt-1 block w-full rounded-md bg-gray-700 border-gray-600 text-white transition-all duration-200 focus:ring-2 focus:ring-blue-500"
      ...
      <template v-if="mode === 'signup'">
        <div class="transform transition-all duration-300">
          ...
          />
          <p v-if="passwordConfirmError" class="mt-1 text-sm text-red-500">{{ passwordConfirmError }}</p>
        </div>
